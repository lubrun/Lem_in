...
/*   Updated: 2019/08/07 06:24:56 by qbarrier    ###    #+. /#+    ###.fr     */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
...
int ft_putheat_max(t_room *room, int heat)
{
	int index;
	int count;
...
int		ft_putcount_heatmax(t_room *room, int heat, char *s_name, char *e_name)
{
	int index;
	int count;
	...
	while (room->link[index])
	{
		...
		{
			if (set_heat_max(room->link[index], heat, s_name, e_name) == -2)
			{
				if (set_heat_max(room, heat, s_name, e_name) == -2)
					return (-2);
				break;
			}
		}
...
int		set_heat_max(t_room *room, int heat, char *s_name, char *e_name)
{
	...
	if ((count = ft_checkerror_heat_max(room, heat)) <= 0)
		return (count);
	heat = ft_putheat_max(room, heat);
	index = 0;
	count = 0;
	if ((count = ft_putcount_heatmax(room, heat, s_name, e_name)) < 0)
		return (count);
